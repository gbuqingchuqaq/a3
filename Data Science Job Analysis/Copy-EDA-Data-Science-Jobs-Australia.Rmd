...
### Let's Import the required libraries to start the analysis.
...
```{r warning=FALSE, message=FALSE,show_col_types = FALSE}
# Loading Dataset
...
Let's convert date features into datetime.
...
<b> Let's start the EDA and preprocessing (if we needed) together.</b>
...
so let's create a new variable called job_description to avoid two field interruption and if we look at the data carefully. we will get to know that some words are joined together in mobileAdTemplate so only desktopAdTemplate make sense and we will continue further EDA with that variable only.
...
Let's create a wordcloud to explore the text content of job descriptions:
...
Although of lot of listings didn't provide any salary information, we now have the salary for 724 of 2857 listings, so we can explore those which we have.
